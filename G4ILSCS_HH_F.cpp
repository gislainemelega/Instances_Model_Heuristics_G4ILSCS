...
//Upper Bound: Hybrid Heuristic: relax-and-fix procedure (product-oriented decomposition - demandF) 
...
// ************************************************** //
		  ...
		  out << "* Upper Bound: Hybrid Heuristic: relax-and-fix procedure (product-oriented decomposition - demandF)" << endl;
...
//The Relax-and-Fix procedure considers a product-oriented decomposition - demandF
//(the final products in a window is according to their sum of their demand)
//For each product-window: the whole planning horizon is considered.
//The binary setup variables related to a cutting pattern is considered in the
//window if it has a piece that belongs to any final product in the window.
//The binary setup variables related to the production of objects and the 
//selection of the supplier are kept binary during all the Hybrid Heuristic.
//The resulting problem (with fixed, integer, and relaxed variables) is solved
...
// *********************************** //
		  ...
		  //Relax-and-Fix procedure with product-oriented decomposition - demandF
		  int F_WindowSize,																			//number of products in the product-window (F_WindowSize = F_Fix + F_Overlap),
			 
			  F_Fix,																			    //number of products in the product-window that are fixed 
		     
			  F_Overlap;													                        //number of products in the product-window that overlap  
		 

		  int F_int_begin, F_int_end;																//interval of products where the binary setup variables are integer
		  ...
		  //with a Product-Oriented Decomposition - demandF

		  //Note: F_WindowSize = F_Fix + F_Overlap 
		  if (F == 10) {		 
						  F_WindowSize = 9;
					      F_Fix = 3;
					      F_Overlap = 6;
					      F_int_begin = 0;
					      F_int_end = F_int_begin + F_WindowSize;
		  }
		  ...
		  out << endl << endl;
		  out << "* Product-oriented decomposition - demandF: F = " << F << endl;
          out << "*                 parameters                product-window = " << F_WindowSize << endl;
          out << "                                            product-fixed = " << F_Fix << endl; 
          out << "                                            product-overlapping = " << F_Overlap << endl;  
		  out << endl << endl;
		  ...
		  //Objects and Suppliers
		  //Because there is no relation between the final products and the 
	      //objects and suppliers, these binary setup variables are not fixed,
		  //instead they are kept binary during all the Hybrid Heuristic
		  ...
		  //to the cutting patterns are created inside of the LOOP
		  //due to the setup decision variables of cutting patterns
		  std::vector<vector<IloConversion> > convYF(T, vector<IloConversion>(F));
		  std::vector<IloConversion> convYO(T);
		  std::vector<IloConversion> convYS(T);

		  for(t=0; t<T; t++)
		     for(f=0; f<F; f++)
		        convYF[t][f] = IloConversion(env, YF[t][f], ILOBOOL);
		  ...
		  for(; ;){
				  ...
				  //Calculate the maximum number of cutting
				  //patterms among all the time periods
				  int max_J = 0;
				  
				  for(t=0; t<T; t++){
				 	 if (max_J <= J[t]) max_J = J[t];
				  }

				  std::vector<vector<IloConversion> > convYJ(T, vector<IloConversion>(max_J));

				  for(t=0; t<T; t++)
				     for(j=0; j<J[t]; j++)
					    convYJ[t][j] = IloConversion(env, YJ[t][j], ILOBOOL);





				  //Consider the integrality of the binary setup variables from 0 to F_int_end
				  //Converte the linear variables to binary variables

				  //Final Product
				  for(t=0; t<T; t++)
		             for(f=0; f < F_int_end; f++)
						MPmodel.add(convYF[t][f]); 
				  ...
				  //Cutting Patterns
				  //The convWJ_aux parameter check if the integrality of a binary setup variable 
				  //related to a cutting pattern (YJ_tj) has been aready inserted in the problem
				  //This is because a cutting patterns is related to more than one piece, and 
				  //a piece can be a component of more than one final product (0-no; 1-yes)
				  IloArray<IloNumArray> convYJ_aux(env, T);
				  for(t=0; t<T; t++)
					 convYJ_aux[t] = IloNumArray(env, J[t], 0, 1, ILOINT);
				  ...
				  for(f=0; f<F_int_end; f++){
					 //For each piece, check if the piece belongs 
					 //to a final product from 0 to F_int_end
					 for(p=0; p<P; p++){
					    if (r[f][p] > 0) {
										    //For each cutting pattern, check if it has the corresponding 
										    //piece and the integrality of the binary setup variable related to
											//this cutting pattern (convYJ_aux[t][j]) has not been added yet
										    for(t=0; t<T; t++){
											   for(j=0; j<J[t]; j++){
												  if ((a[t][p][j] > 0) && (convYJ_aux[t][j] == 0)) {

																//Add the integrality constraints
																MPmodel.add(convYJ[t][j]);
														        
																//Update the integrality constraints added
																convYJ_aux[t][j] = 1;
				  ...
				  if (checkHH_lastWindow == 1) {
										  ...
										  out << endl;
										  out << endl;
										  out << "Number of cutting patterns and cargo configurations" << endl;
										  out << "generated in each time period of the Hybrid Heuristic" << endl;
										  for(t=0; t<T; t++)
				  ...
				  //Recover the binary values of the binary setup variables
				  for(t=0; t<T; t++){

					 //Final Products
					 for(f=0; f < (F_int_begin+F_Fix); f++)
				  ...
				  //Final Products
				  for(t=0; t<T; t++)
					 for(f = F_int_begin; f < (F_int_begin+F_Fix); f++){
					    IloExpr fix(env);
						fix += YF[t][f];
				  ...
				  //Cutting Patterns
				  //The RestFixWJ_aux parameter check if the constraints  
				  //that fix a binary setup variable related to the cutting 
				  //pattern (YJ_tj) has been aready inserted in the problem.
				  //This is because a cutting patterns is related to more than one piece, and 
				  //a piece can be a component of more than one final product (0-no; 1-yes)
				  IloArray<IloNumArray> RestFixYJ_aux(env, T); 
				  for(t=0; t<T; t++)
					 RestFixYJ_aux[t] = IloNumArray(env, J[t]);


				  for(f = F_int_begin; f < (F_int_begin+F_Fix); f++){
					 //For each piece check if the piece belongs 
					 //to a final product from F_int_begin to (F_int_begin+F_Fix)
					 for(p=0; p<P; p++){
						if (r[f][p] > 0) {
											//For each cutting pattern, check if it has the corresponding piece
										    //and the constraint that fix the binary setup variables related to
											//this cutting pattern (RestFixWJ_aux[t][j]) has not been added yet
											for(t=0; t<T; t++){
											   for(j=0; j<J[t]; j++){
												  if ((a[t][p][j] > 0) && (RestFixYJ_aux[t][j] == 0)) {
																			...
																			//Update the fixed constraints added
																			RestFixYJ_aux[t][j] = 1;
												  }//end if
											   }//end j
											}//end t
				  ...
				  for(f=0; f < F_int_begin; f++){
					 //For each piece check if the piece belongs 
					 //to a final product from 0 to F_int_begin
					 for(p=0; p<P; p++){
						if (r[f][p] > 0) {
											//For each cutting pattern generated in the column generation procedure
										    //of the Hybrid Heuristic, check if it has the corresponding piece
											//and the constraint that fix the binary setup variables related to
											//this cutting pattern ((RestFixYJ_aux[t][j]) has not been added yet
											for(t=0; t<T; t++){
				  ...
				  for(t=0; t<T; t++){
					 ...
					 //Final Products
		             for(f=0; f < F_int_end; f++)
					    MPmodel.remove(convYF[t][f]); 

					 //Cutting patterns
				     for(j=0; j<J[t]; j++)
					    if (convYJ_aux[t][j] == 1) MPmodel.remove(convYJ[t][j]); 
				  ...
				  //Move to the next product-window in the Hybrid Heuristic
				  F_int_begin = F_int_end - F_Overlap;
				  F_int_end   = F_int_begin + F_WindowSize;
				  ...
				  //new ones are add from 0 to F_int_end in the beginning of the LOOP,
